...
func mcpTools() []mcpTool {
	...
	tools := []mcpTool{
		...
		klineTool,
		newMCPTool("tdx_stock_brief_text", "单只A股快速概览。适合作为个股分析第一步，不等于深度买卖建议；输出行情、基本面、最新财报、所属板块、估值表现、数据一致性和日/周/月技术指标；递推指标最多使用250根可用K线预热。", "/api/agent/stock-brief-text", handleAgentStockBriefText,
			requiredString("code", "股票代码，例如300499"),
			optionalMarket("mkt"),
		),
		newMCPTool("tdx_kline_summary_text", "K线阶段走势摘要。用于中短线和长期走势判断；level/dayCount控制展示区间，RSI和Wilder ATR等递推指标至少使用250根可用K线预热；输出日线、周线、月线的趋势、涨跌、RSI6、OBV、ATR、波动和风险提示。", "/api/agent/kline-summary-text", handleAgentKlineSummaryText,
			requiredString("code", "股票代码，例如300499"),
			optionalEnumDefault("level", "输出深度：brief为简版，normal为常规版，deep为深度版。默认normal。", "normal", "brief", "normal", "deep"),
			optionalInteger("dayCount", "日线展示与阶段统计数量，最大500；不传时按level使用brief=60、normal=120、deep=250。递推指标仍至少使用250根可用K线预热。", map[string]any{"minimum": 1, "maximum": 500}),
		),
		...
		newMCPTool("tdx_market_review_text", "市场级复盘。用于判断A股整体环境；严格分开输出当前交易日实时广度与最近完整交易日盘后广度，两者均标注日期、时点、A股有效样本数和数据源，不会用昨日数据冒充今日。另含主要指数、强/中/弱板块和可选关注股联动。", "/api/agent/market-review-text", handleAgentMarketReviewText,
		...
		newMCPTool("tdx_technical_score_text", "统一技术评分。递推指标至少使用250根可用K线预热；日线价格指标默认使用Gbbq前复权日K线，周/月线使用现有TDX口径；RSI使用Wilder平滑，KDJ(9,3,3)逐根递推，金叉/死叉通过前后周期K、D关系确认；对MA、MACD、RSI、BOLL、KDJ、BIAS、量价和日线多空比打分。", "/api/agent/technical-score-text", handleAgentTechnicalScoreText,
			requiredString("code", "股票代码，例如300499"),
