...
func mcpTools() []mcpTool {
	...
	tools := []mcpTool{
		...
		klineTool,
		newMCPTool("tdx_stock_brief_text", "单只A股基本快照，不等于深度买卖建议。输出查询时点行情、基本面、最新财报、所属板块、估值和数据一致性；不包含技术指标、5/20/60日涨跌及52周区间。交易时段行情按实时快照，午休使用上午最后行情，盘后使用当日收盘；盘前、非交易日或实时报价为零时回退到最近有效日K收盘，不会把无报价计算为-100%，并明确查询时间、行情数据日期和状态。财务及估值保留各自报告或统计日期。技术判断另调用tdx_technical_score_text，价格历史结构另调用tdx_kline_summary_text。", "/api/agent/stock-brief-text", handleAgentStockBriefText,
		...
		newMCPTool("tdx_margin_trading_text", "个股融资融券数据。直连股票所属交易所官方披露，按最新已披露记录向前取交易日；默认summary输出最新值、区间变化和高低点，适合Agent分析，full才输出逐日明细。交易所可联通但回溯后无该证券明细时，判定当前不是融资融券标的。", "/api/agent/margin-trading-text", handleAgentMarginTradingText,
			...
			optionalEnumDefault("mode", "文本输出模式：summary默认，仅输出最新值、区间变动、最高最低及累计融资净买入；full输出每个交易日的完整明细表。", "summary", "summary", "full"),
		),
		newMCPTool("tdx_f10_summary_text", "低频深度F10摘要。用于深度个股研究和风险线索复核；默认输出全部保留分类，也可用sections精确选择分类以控制上下文。正文不重复分类用途、排除清单和stock-brief估值字段。", "/api/agent/f10-summary-text", handleAgentF10SummaryText,
			requiredString("code", "股票代码，例如300499"),
			optionalMarket("mkt"),
			optionalString("sections", "可选，逗号分隔的精确分类名；例如股东研究,经营分析。留空输出全部保留分类。可选分类：股本结构、股东研究、机构持股、分红融资、资金动向、资本运作、热点题材、公司公告、经营分析、行业分析、研报评级。"),
		),
		...
		newMCPTool("tdx_stock_in_sector_text", "个股在指定所属板块内的相对位置。使用所标TdxStat最近完整统计日数据，输出查询时间、统计日期、目标股排名、百分位和板块前列股票；用于判断目标股相对同板块股票是强势、中游还是落后，不是盘中实时排名。", "/api/agent/stock-in-sector-text", handleAgentStockInSectorText,
			requiredString("code", "股票代码，例如300499"),
		...
		newMCPTool("tdx_market_review_text", "市场级复盘。用于判断A股整体环境；严格分开输出当前交易日实时广度与最近完整交易日盘后广度，两者均标注日期、时点、A股有效样本数和数据源，不会用昨日数据冒充今日。盘前尚未产生当前广度时明确说明，并继续输出上一交易日盘后广度，不视为接口失败。只提供少量强/中/弱板块环境摘要；需要完整板块排序、周期比较和代表股时再调用tdx_hotspot_scan_text。", "/api/agent/market-review-text", handleAgentMarketReviewText,
			...
			optionalIntegerDefault("top", "强/中/弱板块各自展示数量，默认5，范围1-5；完整板块排名请调用tdx_hotspot_scan_text。", 5, 1, 5),
		),
		newMCPTool("tdx_intraday_alerts_text", "关注股盘中异动快照。用于交易时段轮询关注池；输出查询时间，异动股票保留当前行情、短时涨跌、短时放量和信号，无明显异动股票合并为一行，重复数据告警按原因归并。", "/api/agent/intraday-alerts-text", handleAgentIntradayAlertsText,
			requiredString("codes", "逗号分隔股票代码，最多20只"),
			optionalIntegerDefault("windowMinutes", "分时窗口分钟数，默认30，范围5-60。", 30, 5, 60),
		),
		newMCPTool("tdx_global_market_brief_text", "全球外围权重资产简报。用于A股分析前判断外围环境；统一输出生成时间，以及全球指数、亚太市场、商品、汇率、债券和权重股的当日及20/60日表现，不重复每项资产的静态入选理由。", "/api/agent/global-market-brief-text", handleAgentGlobalMarketBriefText),
		newMCPTool("tdx_scenario_valuation_text", "三情景估值计算。复用stock-brief的价格、PE和财务同比；输出价格来源、行情日期与状态、PE统计日期，并计算目标EPS、目标价、涨跌幅和年化收益，不输出买卖建议。", "/api/agent/scenario-valuation-text", handleAgentScenarioValuationText,
			requiredString("code", "股票代码，例如300499"),
			optionalMarket("mkt"),
		...
		newMCPTool("tdx_implied_expectation_text", "当前价格隐含预期计算。用当前价、EPS和目标PE反推未来EPS与隐含年复合增速；输出价格来源、行情日期与状态、PE统计日期和净利润报告期，只做公式计算，不输出买卖建议。", "/api/agent/implied-expectation-text", handleAgentImpliedExpectationText,
			requiredString("code", "股票代码，例如300499"),
			optionalMarket("mkt"),
			optionalIntegerDefault("years", "估值期限，默认3年，范围1-10。", 3, 1, 10),
			optionalNumberSchema("currentPrice", "可选当前价格；必须为正数；不传则使用行情现价。", positiveNumberSchema()),
		...
		newMCPTool("tdx_technical_score_text", "统一技术评分。输出日/周/月MA、MACD、RSI、BOLL、KDJ、BIAS、ATR、OBV、量价，日线另含逐笔成交估算的主动买卖金额多空比；该多空比不是分档资金流，不替代tdx_trade_flow_estimate_text。ATR和OBV为观察项、固定0分，不改变技术总分。输出查询时间及各周期数据日期；交易时段按未完成日K动态计算，午休使用上午最后日K，盘后使用当日收盘，盘前和非交易日使用最近完整交易日。递推指标至少使用250根可用K线预热；RSI使用Wilder平滑，KDJ(9,3,3)逐根递推并通过前后周期确认交叉。通过includeWeeklyMonthly控制是否输出周/月线，无level参数。", "/api/agent/technical-score-text", handleAgentTechnicalScoreText,
			requiredString("code", "股票代码，例如300499"),
